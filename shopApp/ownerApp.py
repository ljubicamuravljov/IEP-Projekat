...
from models import database, Product, ProductCategories, Category, OrderProducts, Order
...
SPARKAPP=os.environ["SPARKAPP_URL"]

@application.route("/category_statistics", methods=["GET"])
@roleCheck(role="owner")
def category_statistics():
    spark_response = requests.get(f'http://{SPARKAPP}:5004/catStat')

    return spark_response.content, spark_response.status_code
@application.route("/product_statistics", methods=["GET"])
@roleCheck(role="owner")
def product_statistics():
    spark_response = requests.get(f'http://{SPARKAPP}:5004/prodStat')
    ...
    #
    #     orders = Product.query.join(OrderProducts).join(Order).filter(Product.id == product.id)
    ...
    #         'name': product.name,
    #         'sold': sold,
    #         'waiting': totalOrders - sold
    #     }
    #     statsList.append(tmpStat)
    #
    # return jsonify(statistics=statsList)
